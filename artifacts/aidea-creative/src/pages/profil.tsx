...
function printInvoicePesanan(p: PesananRow, settings?: SiteSettings) {
  ...
  const statusPesananLabel = ({ diproses: "DIPROSES", dikerjakan: "SEDANG DIKERJAKAN", siap_ambil: "SIAP DIAMBIL", selesai: "SELESAI", dikirim: "DIKIRIM", dibatalkan: "DIBATALKAN" } as Record<string, string>)[p.status] ?? p.status.toUpperCase();
...
export default function Profil() {
  ...
  const payBookingViaWA = (b: BookingRow) => {
    const adminWa = (siteSettings?.contactWhatsapp ?? b.adminWa ?? "").replace(/\D/g, "").replace(/^0/, "62");
    const tanggal = (() => {
  ...
  const siapAmbilCount = pesanan.filter((p) => p.status === "siap_ambil").length;
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full grid grid-cols-4 h-auto">
            ...
            <TabsTrigger value="pesanan" className="gap-1.5 py-2.5 text-xs sm:text-sm">
              ...
              <span>Pesanan</span>
              {siapAmbilCount > 0 && (
                <span className="ml-0.5 min-w-[18px] h-[18px] rounded-full bg-purple-600 text-white text-[10px] font-bold flex items-center justify-center">
                  {siapAmbilCount}
                </span>
              )}
            </TabsTrigger>
                    ...
                    {steps.map((step, i) => {
                      ...
                      return (
                        <div key={step.key} className="flex items-center gap-1 flex-1 last:flex-none">
                          ...
                          {i < steps.length - 1 && (
                            <div className={`h-0.5 flex-1 mb-4 rounded-full ${done ? "bg-emerald-400" : "bg-border"}`} />
                          )}
                        </div>
                      );
                    })}
                ...
                <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 flex gap-2.5">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-blue-700">
                      Kami sedang memproses pesanan Anda. Anda akan diberitahu saat pesanan siap diambil.
                    </p>
                  </div>
                </div>
              )}
